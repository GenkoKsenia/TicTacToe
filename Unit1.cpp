...
{
 Form6->Show();
 Form6->StringGrid1->Cells[0][0] = "№";        // Заполняем шапку таблицы
 Form6->StringGrid1->Cells[0][1] = "1";
 Form6->StringGrid1->Cells[0][2] = "2";
 Form6->StringGrid1->Cells[0][3] = "3";
 Form6->StringGrid1->Cells[1][0] = "Имя";
 Form6->StringGrid1->Cells[2][0] = "Игр";
 Form6->StringGrid1->Cells[3][0] = "Побед";

 ifstream filer1("users.bin", ios::binary); // Открываем файл с данными пользователей в двоичном режиме

 if (filer1.is_open()) {   //Проверяем открытие файла
	UserInfo userrating;
	UserInfo top1; // Создаем три переменные типа UserInfo для хранения информации о трех лучших пользователях
	top1.games=0;
	top1.wins=0;
	UserInfo top2;
	top2.games=0;
